...
SUM(SALARY) "급여 함"
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') -- 별칭 사용 X (SELECT 해석X)
...
FROM EMPLOYEE
GROUP BY DEPT_CODE, JOB_CODE -- DEPT_CODE로 그룹화, 나눠진 그룹내에서 JOB_CODE로 그룹화
ORDER BY DEPT_CODE, JOB_CODE DESC;
--  ORA-00979: GROUP BY 표현식이 아닙니다.

-- GROUP BY 사용 시 주의사항
-- SELECT 문에 GROUP BY절을 사용할 경우
-- SLEECT절에 명시한 조화하려는 컬럼 중
-- 그룹함수가 적용되 잖은 컬럼을
...
FROM EMPLOYEE
--WHERE SALARY >= 3000000 -- 한 사람의 급여가 3백만원 이사이라는 조건(요구사항 X)
GROUP BY  DEPT_CODE
