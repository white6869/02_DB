...
-- '####안녕####' 아래 예시와 같이 사용할 경우 삭제할 값은 반드시 맨앞이나 맨뒤에 삭제할값이 있어야 삭제된다
-- 아래 예시 기준 '####안#녕####' 할 경우 안에 있는 #은 삭제되지 않는다
...
SELECT ADD_MONTHS(SYSDATE, 4) FROM DUAL; -- 2026-02-20 09:30:20.000
SELECT ADD_MONTHS(SYSDATE, -1) FROM DUAL; --2025-09-20 09:31:21.000

-- LAST_DAY(날짜) : 해당 달의 마지막 날짜를 구함
SELECT LAST_DAY(SYSDATE) FROM DUAL; --2025-10-31 09:32:53.000
SELECT LAST_DAY('2020-02-01') FROM DUAL; --2020-02-29 00:00:00.000

-- EXTRACT() : 년, 월, 일 정보를 추출하여 반환
-- EXTRACT(YEAR FORM 날짜) : 년도만 추출
-- EXTRACT(MONTH FORM 날짜) : 월만 추출
-- EXTRACT(DAY FORM 날짜) : ; 일만 추출

-- EMPLOYEE 테이블에서
-- 각 사워의 이름, 입사일 조회(입사년도, 월, 일)
-- 2010년 10월 10일
...
-- 2025/10/20/ 10:06:20 월요일
SELECT TO_CHAR(SYSDATE, 'YYYY/MM/DD HH24:MI:SS DAY') FROM DUAL;
-- 2025/10/20 10:08:24 월요일
...
SELECT TO_DATE('490505', 'RRMMDD') FROM DUAL; --22049-05-05 00:00:00.000
...
-- DECODE(계산식 | 컬럼명, 조건값1, 선택값2, 조건값2, 선택값2, ... , 아무것도 일치하지 않을때)
...
-- 급여 최대값, 가장늦은 입사일, 알파벳 순서가 가장늦은 이메일 조회
...
-- 가장 큰 급여 조회
SELECT MAX(SALARY) FROM EMPLOYEE; -- 서브쿼리와 + 그룹함수
...
-- COUNT() : 행 개수를 헤아려서 반환
-- COUNT(컬ㄹ령) : NULL를 제외한 실제값이 기록된 행 개수를 반환함
