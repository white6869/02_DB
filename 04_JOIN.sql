...
-- 1. 내부 조인(INNER JOOIN) (== 동기조인(EQUAL JOIN))
...
SELECT EMP_ID, EMP,_NAME, EMP_CODE, DEPT_TITLE
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DPET_ID);
...
-- 23GOD (DEPT_CODE가 NULL인 하동운, 이오리 포함)
...
-- ANSI 표준
SELECT E1.EMP_ID 사번, E1.EMP_NAME 사원이름, NVL(E1.MANAGER_ID, '없음') "사수의 사번", 
...
-- 반드시 두 테이블간의 동일한 컬럼영, 타입을 가진 컬럼이 필요
...
SELECT EMP_NO "사번", EMP_NAME "사원명", JOB_NAME "직급명", DEPT_TITLE "부서명"
FROM EMPLOYEE E
JOIN JOB J ON (J.JOB_CODE = E.JOB_CODE )
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
...
SELECT EMP_NAME "사원명", JOB_NAME,DEPT_CODE "부서코드", DEPT_TITLE "부서명"
FROM EMPLOYEE E
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB J ON(J.JOB_CODE  = E.JOB_CODE)
...
SELECT EMP_NAME "사원명", JOB_NAME "직급명", SALARY "급여", (SALARY * '12' *(BONUS + '1.0')) "연봉"
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
JOIN (SELECT E2.JOB_CODE, MIN(SALARY) "MIN_SAL2" FROM EMPLOYEE E2 GROUP BY JOB_CODE ) USING(JOB_CODE)
WHERE MIN_SAL2 < SALARY;

-- 주의사항 
-- NATURAL JOIN을 다중조인에서 사용할 떄는 SELECT 절에 NATURAL JOIN에서
-- 사용 되었을 연결고리 컬럼을 반드시 작성해야함!
