...
SELECT EMP_NAME, DEPT_CODE
FROM EMPLOYEE
WHERE (SELECT DEPT_CODE FROM EMPLOYEE WHERE EMP_NAME = '노옹철') = DEPT_CODE;
...
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
WHERE (SELECT CEIL(AVG(SALARY)) FROM EMPLOYEE) <= SALARY;
...
JOIN JOB USING(JOB_CODE)
WHERE (SELECT SALARY FROM EMPLOYEE WHERE EMP_NAME ='노옹철') < SALARY;
...
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME,
CASE WHEN EMP_ID IN (SELECT MANAGER_ID FROM EMPLOYEE WHERE MANAGER_ID IS NOT NULL)
THEN '사수'
ELSE '사원'
END 구분
FROM EMPLOYEE
...
WHERE EXTRACT(HIRE_DATE, 'YYYY') = 2000);
...
-- 4. 다중행 다중열 서브쿼리
-- 서브쿼리 조회 결과 행 수와 열수가 여러개 일 때

-- 본인이 소속된 직급의 평균 급여를 받고있는 직원의
-- 사번, 이름, 직급코드, 급여 조회
-- 단, 급여와 급여 평균은 만원 다위로 계산 TRUNC(컬럼명, 단위)

-- 1) 직급의 평균 급여 (서브쿼리)
SELECT JOB_CODE, TRUNC(AVG(SALARY), -4) -- -4 : 
FROM EMPLOYEE
GROUP BY JOB_CODE;

-- 2) 사번, 이름, 직급코드, 급여 조히(메인 + 서브)
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
...
FROM EMPLOYEE SUB
WHERE MAIN.JOB_CODE = SUB.JOB.CODE);
...
-- 모든 직원의 이름, 직급, 급여 전체 사원중 가장 높은 급여의 차를 조회
SELECT EMP_NAME, JOB_CODE, SALARY,
...
-- 스칼라 + 상관쿼리
SELECT EMP_NAME, JOB_CODE, SALARY,
( SELECT CEIL(AVG(SALARY)) FROM EMPLOYEE SUB WHERE MAIN.JOB_CODE = SUB.JOB_CODE) 평균
FROM EMPLOYEE MAIN
ORDER BY JOB_CODE;
...
FROM EMPLOYEE
LEFT JOIN DRPFT ON(DEPT_CODE = DEPT_ID)
...
-- 8. WITH
-- 서브쿼리에 이름 붙여주고 사용시 이름을 사용하게 함
-- 인라인뷰로 사용될 서크붜리에 주로 사용됨
-- 실행속도가 빨라진다는 장점이 있음.
...
-- DENSE_RANK() OVER() : 도일한 순위 이후의 등수를 이후 순위로 계산
...
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN JOB USING(JOB_CODE)
WHERE (DEPT_ID, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE
FROM EMPLOYEE
WHERE EMP_NAME = '노옹철')
AND EMP_NAME != '노옹철';
...
-- 6)
SELECT MIN(HIRE_DATE), DEPT_CODE
FROM EMPLOYEE
WHERE ENT_YN = 'N'
GROUP BY DEPT_CODE;
